...
AXON stays agent-agnostic. This matrix describes the supported ways to
...
| AMD GPU | supported | `remote-infra` or `hybrid-local` | Treat AMD acceleration as optional. Keep a CPU fallback path working first, then enable GPU-backed local model serving only if the host stack is already stable. Avoid making AMD-specific runtime setup a prerequisite for basic AXON adoption. | minimal path first; then infra path if the AMD-backed service host is part of the chosen mode |
| NVIDIA GPU | recommended | `full-local` or `remote-infra` | Best fit for heavier local-model workloads. Even here, separate "AXON works" from "GPU acceleration is tuned." The engine should validate on the minimal path before CUDA-specific optimization or larger local models become part of the support claim. | minimal path first; infra path for the final `full-local` or GPU-backed `remote-infra` shape |
