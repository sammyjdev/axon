...
| Real-usage savings vs full-file reads | 90.7% (SUPERSEDED: measured live against local files; paths moved in the 2026-07-26 tree reorg, no longer reproducible) | axon scripts/recall_savings_report.py | per-chunk telemetry counterfactual: unique source files read in full | 100 requests | 2026-07-04 |
| Real-usage savings vs full-file reads (committed snapshot) | 86.8% exact-count (174,358 vs 1,317,041 tokens, n=182); 83.1% conservative floor over all n=349 | benchmarks/recall_savings_snapshot.jsonl via scripts/recall_savings_report.py --snapshot | whitelisted per-request snapshot (opaque ids, integer counts); exact = requests whose files all resolve; floor includes undercounted counterfactuals | 182 exact / 349 total | 2026-07-27 |
