...
| Real-usage savings vs full-file reads | 90.7% (SUPERSEDED by the committed snapshot below; validated in hindsight - the recoverable subset of its window reproduces at 90.5% under the declared remaps) | axon scripts/recall_savings_report.py | per-chunk telemetry counterfactual: unique source files read in full | 100 requests | 2026-07-04 |
| Real-usage savings vs full-file reads (committed snapshot) | 89.5% exact-count (275,222 vs 2,622,254 tokens, n=271); 87.1% conservative floor over all n=418 | benchmarks/recall_savings_snapshot.jsonl via scripts/recall_savings_report.py --snapshot | whitelisted per-request snapshot (opaque ids, integer counts); directory moves declared in benchmarks/recall_savings_remap.json; exact = requests whose files all resolve; floor includes undercounted counterfactuals | 271 exact / 418 total | 2026-07-28 |
