...
def test_runtime_accepts_all_roadmap_modes(monkeypatch, tmp_path, mode: str) -> None:
    monkeypatch.setenv("AXON_ENGINE", str(tmp_path / "engine"))
    monkeypatch.setenv("AXON_VAULT", str(tmp_path / "vault"))
    monkeypatch.setenv("AXON_RUNTIME_MODE", mode)
...
def test_runtime_rejects_unknown_mode(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("AXON_ENGINE", str(tmp_path / "engine"))
    monkeypatch.setenv("AXON_VAULT", str(tmp_path / "vault"))
    monkeypatch.setenv("AXON_RUNTIME_MODE", "planet-scale")

    with pytest.raises(ValueError, match="Invalid AXON_RUNTIME_MODE"):
        load_runtime_config()
