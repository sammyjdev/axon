...
| `PROMETHEUS_ENGINE`         | `<home>/dev/Prometheus` | Caminho do engine               |
...
<home>/dev/Prometheus/    ← este repositório (engine)
├── src/prometheus/
│   ├── cli/          ← pb CLI (typer)
...
git clone <repo> <home>/dev/Prometheus
cd <home>/dev/Prometheus
