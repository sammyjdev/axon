...
async def get_memory(
    ...
    The 'work' context is protected and requires AXON_WORK_CTX=1 env var.
    """
    if ctx in _PROTECTED_CONTEXTS and os.environ.get("AXON_WORK_CTX") != "1":
        logger.warning("Access to '%s' context denied — set AXON_WORK_CTX=1 to enable", ctx)
        return []
...
async def add_memory(content: str, ctx: str = "personal", user_id: str = "sammy") -> str:
    ...
    if ctx in _PROTECTED_CONTEXTS and os.environ.get("AXON_WORK_CTX") != "1":
        raise PermissionError(f"Context '{ctx}' requires AXON_WORK_CTX=1")
