...
- Engine + specs + docs de agentes: `<home>/dev/Prometheus/`
...
1. **Nunca editar** as 6 specs originais (`<home>/dev/Prometheus/prometheus-*.md`). São fonte imutável.
2. **Nunca misturar** dados do vault (`~/vault/`) com código do engine (`<home>/dev/Prometheus/`).
